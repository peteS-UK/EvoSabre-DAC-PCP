	...
	while True :
		try :
			...
			with canvas(device) as draw:
				draw.text((15, 0),"Connecting to LMS", font=font_logo,fill="white")
				if len(lms_name) > 0 :
					draw.text((20, 22),"LMS Name: " + lms_name, font=font_logo,fill="white")
				draw.text((20, 44),"LMS IP:  " + lms_ip, font=font_logo,fill="white")
			time.sleep(1)
...
#				draw.text((5, 20),"E :" + str(e), font=font_debug,fill="white")
				draw.text((15, 0),"Player Not Connected", font=font_logo,fill="white")
				draw.text((13, 22),"MAC: " + player_mac, font=font_logo,fill="white")
				draw.text((20, 44),"LMS: " + lms_ip, font=font_logo,fill="white")
			time.sleep(1)
		else :
			break
...
# Continuous update			
		...
		if timer_vol > 0 :
			with canvas(device) as draw:
				...
				# Volume Display
				draw.text((0, 25), text="\uf028", font=awesomefontbig, fill="white")
				draw.text((x_vol, -10), volume_val, font=font_vol, fill="white")
				# Volume Bar
		...
		# Play screen
			...
			with canvas(device) as draw:
				if current_page < 150 :	
					...
					draw.text((x_bitrate, 25), (sample_size_val + sample_rate_val + bitrate_val + " " + filetype_val), font=font_ip, fill="white")					
					if info_state == "pause": 
						draw.text((0, 52), text="\uf04c", font=awesomefont, fill="white")
					else:
						draw.text((1, 48), time_val, font=font_time, fill="white")
						draw.text((55, 48), dura_val, font=font_time, fill="white")
					...
					if get_fixed_volume() == False :
						...
						draw.text((101, 48), volume_val, font=font_time, fill="white")
			...
			if screen_sleep < 20000 :
				with canvas(device) as draw:
					if is_wifi == False :
						# LAN IP Address
						draw.text((18, 29), player_ip, font=font_ip, fill="white")
					...
					if get_fixed_volume() == False :
						draw.text((19, 48), volume_val, font=font_time, fill="white")
