...
if (
    ...
    or helper.process_params("LOGFILE") == "INFO"
    or helper.process_params("LOGFILE") == "DEBUG"
):
...
if oled == 0:
    logger.critical(
        "OLED Type must be specified with OLED= parameter or in oled4pcp.cfg"
    )
    exit()
...
display.title_line3_time_xy = helper.parse_int_tuple(
    config[oled]["title_line3_time_xy"]
)
display.title_line3_volume_icon_xy = helper.parse_int_tuple(
    config[oled]["title_line3_volume_icon_xy"]
)
display.title_line3_volume_val_xy = helper.parse_int_tuple(
    config[oled]["title_line3_volume_val_xy"]
)
...
display.title_artist_line_1_font_size = int(
    config[oled]["title_artist_line_1_font_size"]
)
display.title_artist_line_2_font_size = int(
    config[oled]["title_artist_line_2_font_size"]
)
...
if location != "":
    ...
    float(config["CONFIG"]["longitude"]) != 0
    and float(config["CONFIG"]["longitude"]) != 0
):
...
font_title_artist_line_1 = helper.make_font(
    display.font_metadata, display.title_artist_line_1_font_size
)
font_title_artist_line_2 = helper.make_font(
    display.font_metadata, display.title_artist_line_2_font_size
)
...
font_connecting = helper.make_font(
    display.font_connecting, display.connecting_font_size
)

font_banner_logo = helper.make_font(
    ...
    while True:
        try:
            ...
            with canvas(device) as draw:
                ...
                helper.draw_multiline_text_centered(
                    draw, connecting_text, font_connecting, display
                )
            time.sleep(1)
            ...
            subscription_server_handle = helper.LMSTelnetServer(
                hostname=lms_ip,
                port=port,
                username="user",
                password="password",
...
if bufsize >= 8192:
    ...
    with canvas(device) as draw:
        helper.draw_multiline_text_centered(
            draw, display.banner_text, font_banner_logo, display
        )
...
ip_screen_composition.add_image(
    ComposableImage(
        helper.TextImage(device, network_logo, font=font_logo, fill=lightfill).image,
...
try:
    while True:
        ...
        if (
            len(subscription_event) > 0
            and subscription_event[0 : len(player_mac)] == player_mac
        ):
            logger.debug(
                "Subscription Event : %s", subscription_event[len(player_mac) + 1 :]
            )
            get_metadata()
            lastrun_time = int(time.time() * 1000)
        # Update song data no more than once per 375 milliseconds when not stopped
        elif (
            (int(time.time() * 1000) - lastrun_time) > display.playing_polling_interval
        ) and song_data.mode != "stop":
            try:
                get_metadata()
            except Exception as e:
                logger.info("e : %s", e)
        ...
        # Update song data every 10 secs
            ...
            int(time.time() * 1000) - lastrun_time
        ) > display.stopped_polling_interval:
        ...
        if (
            ...
            or mode_store != song_data.mode
            or (
                song_data.sample_size
                + song_data.sample_rate
                + str(song_data.bitrate)
                + song_data.file_type
            )
            != file_info_store
        ):
            ...
            ci_play_line_1 = ComposableImage(
                helper.TextImage(
                    device, song_data.title, font=font_title_artist_line_1
                ).image,
                position=(0, display.title_artist_line1_y),
            )
            scroll_play_line_1 = helper.Scroller(
                play_screen_composition,
                ci_play_line_1,
                20,
                synchroniser,
            ...
            ci_play_line_2 = ComposableImage(
                helper.TextImage(
                    ...
                    + " "
                    + song_data.file_type,
                    font=font_title_artist_line_2,
                    fill=lightfill,
                ).image,
                position=(0, display.title_artist_line2_y),
            )

            scroll_play_line_2 = helper.Scroller(
                play_screen_composition,
                ci_play_line_2,
                20,
                synchroniser,
        ...
        if timer_vol > 0:
            ...
            with canvas(device) as draw:
                ...
                # Volume Display
                draw.text(
                    display.vol_screen_icon_xy,
                    volume_logo,
                    font=font_logo_large,
                    fill="white",
                )
                draw.text(
                    (x_vol, display.vol_screen_value_y),
                    song_data.volume,
                    font=font_vol_large,
                    fill="white",
                )
        ...
        # Play screen
            ...
            if cycle_count == 150 and permit_screen_change == True:
                ...
                ci_play_line_1 = ComposableImage(
                    helper.TextImage(
                        device, song_data.artist, font=font_title_artist_line_1
                    ).image,
                    position=(0, display.title_artist_line1_y),
                )
                ...
                ci_play_line_2 = ComposableImage(
                    helper.TextImage(
                        device, song_data.album, font=font_title_artist_line_2
                    ).image,
                    position=(0, display.title_artist_line2_y),
                )
            ...
            if (cycle_count > 300 and current_TA_page == "artist") or (
            ...
            with canvas(device, background=play_screen_composition()) as draw:
                ...
                if song_data.mode == "pause":
                    draw.text(
                        display.pause_xy, text=pause_logo, font=font_logo, fill="white"
                    )
                else:
                    draw.text(
                        display.title_line3_time_xy,
                        time_val + dura_val,
                        font=font_info,
                        fill=lightfill,
                    )
                ...
                draw.rectangle(
                    (
                        display.title_timebar[0],
                        display.title_timebar[1],
                        time_bar,
                        display.title_timebar[3],
                    ),
                    outline=0,
                    fill=lightfill,
                )
            ...
            if screen_sleep < display.screensave_timeout:
                ...
                with canvas(device) as draw:
                    if screen_sleep == display.screensave_timeout:
                        ...
                        try:
                            if (
                                int(display.height / screensave_char_index)
                                <= display.height - screensave_height
                            ):
                                screensave_xy = (
                                    screensave_xy[0],
                                    int(display.height / screensave_char_index),
                    ...
                    draw.text(
                        screensave_xy,
                        screensave_chars[screensave_char_index],
                        font=font_info,
                        fill="white",
                    )
    ...
    logger.critical("Traceback : %s", "".join(traceback.format_tb(e.__traceback__)))
