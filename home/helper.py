...
def get_sunrise_data(lat, lng):
    ...
    ), datetime.fromisoformat(response["results"]["sunset"])
...
def read_config():
    config_path = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "oled4pcp.cfg")
    )
...
def draw_multiline_text_centered(draw, text, myfont, display):
    ...
    char, char, text_width, text_height = draw.multiline_textbbox(
        (0, 0), text, font=myfont
    )

    draw.text(
        ((display.width - text_width) / 2, (display.height - text_height) / 2),
        text,
        font=myfont,
        fill="white",
        align="center",
...
class LMSTelnetServer(object):
    ...
    def __init__(
        self, hostname="localhost", port=9090, username="", password="", charset="utf8"
    ):
        """
        Constructor
        """
    ...
    def request(self, command_string, preserve_encoding=False):
        ...
        if not preserve_encoding:
            ...
            command_string_quoted = command_string[
                0 : command_string.find(":")
...
def get_player_ip(default_gateway_interface):
    get_ip = netifaces.ifaddresses(default_gateway_interface)[netifaces.AF_INET][0][
        "addr"
    ]
    if get_ip == "":
        get_ip = "127.0.0.1"
